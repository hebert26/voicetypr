    ...
    try {
      ...
      if (showToasts && mic && accessibility) {
        ...
        if (hadDeniedPermission) {
          toast.success("All permissions granted! You're ready to use Verity.");
        }
      }
    } catch (err) {
      const error =
        err instanceof Error ? err : new Error("Failed to check permissions");
      setError(error);
    ...
    try {
      ...
      const error =
        err instanceof Error
          ? err
          : new Error(`Failed to request ${type} permission`);
      setError(error);
